...
namespace ChatBubbleClientWPF.Models
{
    public class ServerFlagReceiver
    {
        ...
        public void DispatchServerFlags()
        {
            while(SharedNetworkConfiguration.MainSocket.Connected)
            {
                ...
                if (serverReply.NetFlag == ConnectionCodes.RecipientFormingReplyStatus)
                    ServerFlagReceived?.Invoke(this, new ServerFlagEventArgs(ServerFlagEventArgs.FlagTypes.RecipientWritingReply, serverReply.Target));
                if (serverReply.NetFlag == ConnectionCodes.RecipientStoppedFormingReplyStatus)
                    ServerFlagReceived?.Invoke(this, new ServerFlagEventArgs(ServerFlagEventArgs.FlagTypes.RecipientStoppedWritingReply, serverReply.Target));
            }
        }
    }
    ...
    {
        public enum FlagTypes { MessagesPending, PendingFriend, MessageStatusReceived, MessageStatusRead, RecipientWritingReply, RecipientStoppedWritingReply }
